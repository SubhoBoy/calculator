...
#define H  0.01
#define MAX_D 2 // Max dimensions for ODE system
...
// --- RK4 Generic Solver (N-Dimensions) ---
// Handles fixed steps + remainder step automatically
void solve(double t, double t_end, double *y, int dim, sys_f f, void *ctx) {
    double dy[MAX_D], k1[MAX_D], k2[MAX_D], k3[MAX_D], k4[MAX_D], yt[MAX_D];
    double dt = t_end - t;
    int n = (int)(m_abs(dt) / H);
    double h = (dt < 0) ? -H : H;

    for (int step = 0; step <= n; step++) {
        // Handle remainder on final extra step
        if (step == n) {
            h = t_end - t;
...
double m_sqrt(double n) { return 1.0 / Q_rsqrt((float)n); }
...
int main(int argc, char *argv[]) {
    if (argc < 3) {
        printf("Usage: %s <id> <val> [opt: pow]\n", argv[0]);
        return 1;
    }

    int sel = atoi(argv[1]);
    double v = atof(argv[2]); // Fixed: parse as double
    double p = 0;

    if (sel == 6) {
        if (argc < 4) { printf("Err: Power needs 2nd arg\n"); return 1; }
        p = atof(argv[3]); // Safe access
    }
