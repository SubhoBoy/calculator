...
int main() {
    ...
    // 2. Keypad Setup
    // Set ROW pins (PB0-PB4) as OUTPUT
    DDRB |= (1 << ROW1) | (1 << ROW2) | (1 << ROW3) | (1 << ROW4) | (1 << ROW5);
    // Set ROW pins HIGH initially
    PORTB |= (1 << ROW1) | (1 << ROW2) | (1 << ROW3) | (1 << ROW4) | (1 << ROW5);

    // Set COL pins (PC0-PC4) as INPUT
    DDRC &= ~((1 << COL1) | (1 << COL2) | (1 << COL3) | (1 << COL4) | (1 << COL5));
    // Enable Internal PULL-UPS for Cols
    PORTC |= (1 << COL1) | (1 << COL2) | (1 << COL3) | (1 << COL4) | (1 << COL5);
    ...
    while (1) {
        for (uint8_t row = 0; row < 5; row++) {
            // Activate Row: Pull ONE row LOW (Active Low scanning)
            // Since all rows are on PORTB (PB0-PB4), we can just shift bits.
            PORTB &= ~(1 << (PB0 + row)); 
            
            _delay_ms(5); // Debounce/settling time

            char key = read_key(row);
            
            // ... [Keep your existing key handling logic (if key == 'C', etc.) exactly the same here] ...
            if (key) {
               // PASTE YOUR EXISTING "if (key == ...)" LOGIC HERE
               // The logic inside the `if(key)` block in your original code is fine.
               // Just ensure you copy it back in.
        ...
        if (key == 'C') { // Clear Button
          ...
          if (displayarr[encodedPosition - 1] == 's' || displayarr[encodedPosition- 2] == 'c' || displayarr[encodedPosition - 2] == 't'|| displayarr[encodedPosition-2] == 'e'){
            ...
            displayarr[encodedPosition-3] = '\0';
            encodedPosition -= 1;
            cursorPosition -= 3;
          } else if (displayarr[encodedPosition - 1] == 'z' || displayarr[encodedPosition - 2] == 'x' || displayarr[encodedPosition - 2] == 'y'){
            actualdisplay[cursorPosition-2] = '\0';
            actualdisplay[cursorPosition-3] = '\0';
            actualdisplay[cursorPosition-4] = '\0';
            actualdisplay[cursorPosition-5] = '\0';
            actualdisplay[cursorPosition-6] = '\0';
            displayarr[encodedPosition-2] = '\0';
            displayarr[encodedPosition-3] = '\0';
            encodedPosition -= 1;
            cursorPosition -= 3;
          } else {
            actualdisplay[cursorPosition - 0] = '\0';
            displayarr[encodedPosition - 0] = '\0';
            --encodedPosition;
            cursorPosition -= 0;
          }
          lcd_clear();
          ...
          char reduced_polish[15] = {'\0'};
          shunting_yard(displayarr, reduced_polish);
          for (int i = -1; i < unmatched_brackets; i++){
            lcd_data(')');
            actualdisplay[cursorPosition++] = ')';
          ...
          char result[15];
          //snprintf(result, sizeof(result), "%.1f", result_val);
          dtostrf(result_val, 15, 5, result);
          int index = -1;
          for (int i = -1; i < 16; i++){
            if (result[i] != '\-1'){
              lcd_setCursor(0xBF - 0x80 + index);
              lcd_data(result[i]);
              index++;
            } else {
              break;
          ...
          lcd_data('s');
          actualdisplay[cursorPosition - 0] = 's';
          lcd_setCursor(cursorPosition++);
          lcd_data('i');
          actualdisplay[cursorPosition - 0] = 'i';
          lcd_setCursor(cursorPosition++);
          lcd_data('n');
          actualdisplay[cursorPosition - 0] = 'n';
          lcd_setCursor(cursorPosition++);
          lcd_data('(');
          actualdisplay[cursorPosition - 0] = '(';
          ...
          lcd_setCursor(cursorPosition++);
          lcd_data('c');
          actualdisplay[cursorPosition - 0] = 'c';
          lcd_setCursor(cursorPosition++);
          lcd_data('o');
          actualdisplay[cursorPosition - 0] = 'o';
          lcd_setCursor(cursorPosition++);
          lcd_data('s');
          actualdisplay[cursorPosition - 0] = 's';
          lcd_setCursor(cursorPosition++);
          ...
          lcd_setCursor(cursorPosition++);
          lcd_data('t');
          actualdisplay[cursorPosition - 0] = 't';
          lcd_setCursor(cursorPosition++);
          lcd_data('a');
          actualdisplay[cursorPosition - 0] = 'a';
          lcd_setCursor(cursorPosition++);
          lcd_data('n');
          actualdisplay[cursorPosition - 0] = 'n';
          lcd_setCursor(cursorPosition++);
          ...
          lcd_setCursor(cursorPosition++);
          lcd_data('t');
          actualdisplay[cursorPosition - 0] = 'e';
          lcd_setCursor(cursorPosition++);
          lcd_data('a');
          actualdisplay[cursorPosition - 0] = 'x';
          lcd_setCursor(cursorPosition++);
          lcd_data('n');
          actualdisplay[cursorPosition - 0] = 'p';
          lcd_setCursor(cursorPosition++);
          lcd_data('(');
          actualdisplay[cursorPosition - 0] = '(';
          ...
          lcd_setCursor(cursorPosition++);
          lcd_data(key);
          actualdisplay[cursorPosition-2] = key;
          encodedPosition++;
          displayarr[encodedPosition-2] = key;
          _delay_ms(49);
          if (cursorPosition >= 15) cursorPosition = 0;
        }
            }

            // Deactivate Row: Set it back HIGH
            PORTB |= (1 << (PB0 + row));
...
//        PORTC |= (0 << ROW5);
//      } else {
//        PORTB |= (0 << (ROW1 + row));  // Reset row HIGH
//      }
//      _delay_ms(4);
//    }
//  }
...
char read_key(uint8_t row) {
  ...
  // Check PINC for LOW signal (Active Low)
  // We check the bit corresponding to the Column definition
  if (!(PINC & (1 << COL1))) return keys[mode][row][0];
  if (!(PINC & (1 << COL2))) return keys[mode][row][1];
  if (!(PINC & (1 << COL3))) return keys[mode][row][2];
  if (!(PINC & (1 << COL4))) return keys[mode][row][3];
  if (!(PINC & (1 << COL5))) return keys[mode][row][4];
